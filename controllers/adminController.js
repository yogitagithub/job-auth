...
//work location
exports.createWorkLoc = async (req, res) => {
  try {
    const name = (req.body?.name || "").trim();
    if (!name) return res.status(400).json({ status: false, message: "name is required" });
    ...
    if (existing) {
      ...
      return res.status(409).json({ status: false, message: "Work location already exists" });
    }

    const doc = await WorkLocation.create({ name });
    return res.status(201).json({
      status: true,
    ...
    console.error("createWorkLoc error:", err);
    return res.status(500).json({ status: false, message: "Server error" });
  }
};
...
exports.updateWorkLoc = async (req, res) => {
  try {
    const { id } = req.body || {};
    const name = (req.body?.name || "").trim();
    ...
    const doc = await WorkLocation.findById(id);
    if (!doc) {
      return res.status(404).json({ status: false, message: "Work location not found" });
    }
    ...
    if (doc.isDeleted) {
      return res.status(400).json({
        status: false,
        message: "This work location is already soft deleted and cannot be updated."
      });
    }

    // Duplicate guard among active docs (case-insensitive exact match)
    const dupe = await WorkLocation.findOne({
      _id: { $ne: id },
      isDeleted: false,
      name: { $regex: `^${escapeRegex(name)}$`, $options: "i" }
    });
    if (dupe) {
      return res.status(409).json({
        status: false,
        message: "Another work location with this name already exists"
      });
    }
    ...
    console.error("updateWorkLoc error:", err);
    return res.status(500).json({ status: false, message: "Server error", error: err.message });
  }
};
...
//get work location for admin
exports.getWorkLoc = async (req, res) => {
  try {
    const page = Math.max(parseInt(req.query.page, 10) || 1, 1);
    const limit = Math.min(Math.max(parseInt(req.query.limit, 10) || 10, 1), 100);
    ...
    const [totalRecord, data] = await Promise.all([
      WorkLocation.countDocuments(filter),
      WorkLocation.find(filter)
        .sort({ createdAt: -1, _id: 1 }) // stable internal order
        .skip((page - 1) * limit)
        .limit(limit)
        .lean()
    ...
    return res.status(200).json({
      status: true,
      message: "Work location data fetched successfully.",
      totalRecord,
      totalPage: Math.ceil(totalRecord / limit) || 0,
      currentPage: page,
      data
    ...
    console.error("getWorkLoc error:", err);
    return res.status(500).json({ status: false, message: "Server error" });
  }
};
...
exports.deleteWorkLoc = async (req, res) => {
  try {
    const { id } = req.body || {};
    if (!id || !mongoose.isValidObjectId(id)) {
      return res.status(400).json({
    ...
    console.error("deleteWorkLoc error:", err);
    return res.status(500).json({
      status: false,
      message: "Server error",
      error: err.message
...
//get work location for employer and job seeker
exports.getWorkLocBasedOnRole = async (req, res) => {
  try {
    const role = req.user?.role;
    ...
    return res.status(200).json({
      status: true,
      message: data.length ? "Work location fetched successfully." : "No work location found.",
      totalRecord,
      totalPage: Math.ceil(totalRecord / limit) || 0,
      currentPage: page,
      data
    ...
    console.error("getWorkLocBasedOnRole error:", err);
    return res.status(500).json({ status: false, message: "Server error", error: err.message });
  }
};
